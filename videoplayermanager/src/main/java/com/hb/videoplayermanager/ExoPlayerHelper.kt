...
class ExoPlayerHelper {
    companion object {
        ...
        fun buildEncryptedMediaSource(context: Context, uri: Uri, config: EncryptionConfig): MediaSource {
            val dsf = DefaultDataSourceFactory(context, Util.getUserAgent(context, "ExoPlayerInfo"))
            val mediaSource = ProgressiveMediaSource.Factory(
                    EncryptedFileDataSourceFactory(dsf.createDataSource(), getCipher(config))).createMediaSource(uri)
            return mediaSource
        }
        ...
        private fun getCipher(config: EncryptionConfig): Cipher {
            ...
            aes.init(Cipher.ENCRYPT_MODE, SecretKeySpec(config.key.toByteArray(Charset.defaultCharset()), "ARC4"));
            return aes
        }


        fun playVideo(context: Context, url: String, exoplayerView: CustomExoPlayerView): SimpleExoPlayer {
